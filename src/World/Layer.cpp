...
void Layer::CreatePhysicsActors(apex::ContactListener* contactListener)
{
	...
	for (int y = 0; y < m_Height; ++y)
	{
		for (int x = 0; x < m_Width; ++x)
		{
			...
			{
				apex::PhysicsActor* newActor = new apex::PhysicsActor(sf::Vector2f((x + 0.5f) * tileSize, (y + 0.5f) * tileSize), b2BodyType::b2_staticBody);
				...
				collisionFilter.categoryBits = int(ActorID::BUILDING);
				newActor->SetUserData(int(ActorID::BUILDING));

				collisionFilter.maskBits = int(ActorID::BULLET) | int(ActorID::PLAYER) | int(ActorID::SHEEP);
