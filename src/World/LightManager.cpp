...
void LightManager::SaveLightData()
{
	...
	for (size_t i = 0; i < m_StaticLights.size(); ++i)
	{
		...
		light["blur"] = apex::Math::GetFloatPrecision(m_StaticLights[i].blur, 2);
		light["color"] = apex::ColorToString(m_StaticLights[i].color);
		light["flickers"] = m_StaticLights[i].flickers;
		light["opacity"] = apex::Math::GetFloatPrecision(m_StaticLights[i].opacity, 2);
		...
		light["radius"] = apex::Math::GetFloatPrecision(m_StaticLights[i].radius, 2);
...
bool LightManager::OnButtonPress(sf::Event::MouseButtonEvent buttonEvent)
{
	if (m_IsShowingEditor)
	{
		...
		case  sf::Mouse::Left:
		{
			for (size_t i = 0; i < m_StaticLights.size(); ++i)
			{
				const float dist = apex::Math::Length(mousePosWorldSpace - m_StaticLights[i].position);
		...
		case sf::Mouse::Middle: // Create/destroy lights
		{
				...
				m_CurrentLightRadiusIndex == -1)
			{
				...
				for (size_t i = 0; i < m_StaticLights.size(); ++i)
				{
					if (apex::Math::Length(mousePosWorldSpace - m_StaticLights[i].position) < m_StaticLights[i].radius / 2.0f)
					{
